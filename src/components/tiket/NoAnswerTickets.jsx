...
function NoAnswerTickets({ portalId }) {
  ...
  useEffect(() => {
    const fetchNoAnswerTickets = async () => {
      setLoading(true);
      try {
        const res = await axiosInstance.get(`/api/v1/portals/${portalId}/ticket/no-answer`);
  ...
  return (
      <div className="mt-3">
        <h6 className="mb-2">Заявки без ответа:</h6>
        ...
        {!loading && ticketIds.length === 0 && !error && (
            <p className="text-muted">Нет заявок без ответа.</p>
        )}
      </div>
  );
}

export default NoAnswerTickets;
